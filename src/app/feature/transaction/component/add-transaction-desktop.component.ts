...
@Component({
  ...
  template: `
    <div class="w-full flex items-center justify-center p-6 animate-fade-slide-up">
      <div class="w-full max-w-5xl">
        <div class="bg-background-color rounded-3xl shadow-xl border border-secondary-color">
          <form [formGroup]="transactionForm" class="p-12" (ngSubmit)="handleSubmit()">
            ...
            <div class="mb-10 animate-fade-slide-up" style="animation-delay:0ms">
              <div class="flex items-center mb-6">
                <div class="flex-shrink-0 w-10 h-10 flex items-center justify-center bg-card-color rounded-lg">
                  <span class="text-xl font-bold text-text-card-color">1</span>
                </div>
              ...
              <div class="grid grid-cols-3 gap-8 bg-primary-color rounded-2xl p-8 border border-secondary-color">
                <div class="col-span-3">
                  ...
                  <input type="text" placeholder="Contoh: Makan di restoran"
                    class="w-full rounded-xl p-4 border-2 border-secondary-color focus:border-card-color focus:ring-1 focus:ring-card-color focus:outline-none transition text-base bg-background-color text-text-color"
                    formControlName="title" />
                </div>
                <div>
                  <label class="block text-sm font-semibold text-text-color mb-3">Tipe</label>
                  <select class="w-full rounded-xl p-4 border-2 border-secondary-color focus:border-card-color focus:ring-1 focus:ring-card-color focus:outline-none transition text-base bg-background-color text-text-color"
                ...
                <div>
                  ...
                  <input type="text" placeholder="0" (input)="formatAmount($event)"
                    class="w-full rounded-xl p-4 border-2 border-secondary-color focus:border-card-color focus:ring-1 focus:ring-card-color focus:outline-none transition text-base bg-background-color text-text-color"
            ...
            <div class="mb-10 animate-fade-slide-up" style="animation-delay:100ms">
              <div class="flex items-center mb-6">
                <div class="flex-shrink-0 w-10 h-10 flex items-center justify-center bg-card-color rounded-lg">
                  <span class="text-xl font-bold text-text-card-color">2</span>
                </div>
              ...
              <div class="bg-primary-color rounded-2xl p-8 border border-secondary-color">
                ...
                <textarea placeholder="Catatan detail transaksi"
                  class="w-full rounded-xl p-4 border-2 border-secondary-color focus:border-card-color focus:ring-1 focus:ring-card-color focus:outline-none transition text-base bg-background-color text-text-color"
                  rows="5" formControlName="description"></textarea>
              </div>
            </div>

            <div class="bg-primary-color rounded-2xl p-8 mb-10 border-2 border-secondary-color animate-fade-slide-up" style="animation-delay:200ms">
              ...
              <div class="grid grid-cols-3 gap-6">
                ...
                <div class="bg-primary-color rounded-xl p-4 shadow-sm">
                  ...
                  <p class="text-2xl font-bold" [ngClass]="transactionForm.valid ? 'text-card-color' : 'text-secondary-color'">
                    {{ transactionForm.valid ? '✓ Siap' : '⚠ Belum' }}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div class="flex gap-4 pt-8 border-t-2 border-secondary-color">
              <button type="submit" [disabled]="!transactionForm.valid"
                class="flex-1 cursor-pointer rounded-xl bg-button-color hover:opacity-90 disabled:opacity-50 px-8 py-4 font-semibold text-text-card-color text-lg transition shadow-lg">
                ✓ Tambah Transaksi
              </button>
              <button type="button" (click)="goBack()"
                class="flex-1 cursor-pointer rounded-xl bg-primary-color hover:opacity-80 px-8 py-4 font-semibold text-text-color text-lg transition border border-secondary-color">
                ✕ Batal
              </button>
            </div>
